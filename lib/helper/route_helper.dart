...
class RouteHelper {
  ...
  static String getMemberRoute({RouteAction? action}) => _navigateRoute(member, route: action);
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          OrderModel? orderModel = state.uri.queryParameters['order_details'] == 'null' ? null : OrderModel.fromJson(jsonDecode(utf8.decode(base64Decode('${state.uri.queryParameters['order_details']?.replaceAll(' ', '+')}'))));

          return _routeHandler(
            context,
            child: OrderDetailsScreen(orderId: orderId, orderModel: orderModel, phoneNumber: phone, token: token, flag: flag),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: MenuScreen(isReload: isLoad ?? true, fromIntro: fromInto ?? false),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: OtpRegistrationScreen(tempToken: tempToken ?? '', userInput: input ?? '', userName: userName),
          );
        },
      ),
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: VerificationScreen(fromPage: page, userInput: userInput, session: sessionStr == 'null' ? null : utf8.decode(base64Url.decode(sessionStr.replaceAll(' ', '+')))),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          return _routeHandler(
            context,
            child: CreateNewPasswordScreen(userInput: Uri.decodeComponent(state.uri.queryParameters['email'] ?? ''), resetToken: state.uri.queryParameters['token'] ?? ''),
          );
        },
      ),
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(context, child: AddNewAddressScreen(isEnableUpdate: true, fromCheckout: false, address: AddressModel.fromJson(jsonDecode(decoded))));
        },
      ),
      GoRoute(
        path: selectLocation,
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: OrderSuccessScreen(orderID: state.uri.queryParameters['id'] ?? '', status: status),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          return _routeHandler(
            context,
            child: PaymentScreen(orderId: int.tryParse(state.uri.queryParameters['id'] ?? ''), url: Uri.decodeComponent(utf8.decode(base64Decode(state.uri.queryParameters['uri'] ?? '')))),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          return _routeHandler(
            context,
            child: TrackOrderScreen(orderID: state.uri.queryParameters['id'] ?? '', phone: Uri.decodeComponent(state.uri.queryParameters['phone'] ?? '')),
          );
        },
      ),
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: TrackMapScreen(orderId: int.tryParse(state.uri.queryParameters['order'] ?? ''), deliverymanId: int.tryParse(state.uri.queryParameters['deliveryman'] ?? ''), order: order),
          );
        },
      ),
      GoRoute(
        ...
        builder: (context, state) => _routeHandler(
          context,
          child: CategoryProductScreen(categoryId: state.uri.queryParameters['category_id'] ?? '', categoryName: state.uri.queryParameters['category_name'] ?? ''),
        ),
      ),
      GoRoute(
        path: productDetails,
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: ProductDetailsScreen(productId: state.uri.queryParameters['product_id'], fromSearch: fromSearch),
          );
        },
      ),
      GoRoute(
        ...
        builder: (context, state) {
          return _routeHandler(
            context,
            child: ProductImageScreen(title: state.uri.queryParameters['name'], baseUrl: Uri.decodeComponent(state.uri.queryParameters['base_url'] ?? ''), imageList: List<String>.from((jsonDecode(state.uri.queryParameters['images'] ?? '[]') as List<dynamic>))),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: ChatScreen(orderId: state.uri.queryParameters['order_id'] ?? '', userName: state.uri.queryParameters['name'] ?? '', senderType: state.uri.queryParameters['type'] ?? '', profileImage: state.uri.queryParameters['image'] ?? '', isAppBar: isAppbar),
          );
        },
        redirect: (context, state) {
          final bool isDeliveryman = (state.uri.queryParameters['type'] ?? '').contains('deliveryman');
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return _routeHandler(
            context,
            child: AddNewAddressScreen(fromCheckout: state.uri.queryParameters['page'] == 'checkout', isEnableUpdate: isUpdate, address: isUpdate ? addressModel : null),
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) => _routeHandler(
          context,
          child: WalletScreen(token: state.uri.queryParameters['token'] ?? '', status: state.uri.queryParameters['flag'] ?? ''),
        ),
        redirect: (context, state) {
      ...
      GoRoute(
        path: member,
        builder: (context, state) => _routeHandler(context, child: const MemberScreen()),
        redirect: (context, state) {
          return _redirectUrl(context, state, 0);
        },
      ),
      GoRoute(
        path: referAndEarn,
      ...
      GoRoute(
        ...
        builder: (context, state) {
          return _routeHandler(
            context,
            child: PreviewScreen(images: List<String>.from((jsonDecode(state.uri.queryParameters['images'] ?? '[]') as List<dynamic>)), selectedIndex: int.tryParse(state.uri.queryParameters['index'] ?? '') ?? 0),
          );
        },
      ),
      GoRoute(
        path: orderMapInfo,
        builder: (context, state) {
          DeliveryAddress? deliveryAddress = state.uri.queryParameters['delivery_address'] == 'null' ? null : DeliveryAddress.fromJson(jsonDecode(utf8.decode(base64Decode('${state.uri.queryParameters['delivery_address']?.replaceAll(' ', '+')}'))));

          return _routeHandler(context, child: OrderMapInfoWidget(address: deliveryAddress));
        },
      ),
    ],
  );
  ...
  static Widget _routeHandler(BuildContext context, {required Widget child}) {
    return isMaintenance(Provider.of<SplashProvider>(context, listen: false).configModel) ? const MaintenanceScreen() : child;
  }
