...
class MenuListWebWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<MenuModel> menuList = [
      ...
      MenuModel(icon: Images.editProfile, title: getTranslated('profile', context), route: () => isLoggedIn ? RouteHelper.getProfileEditRoute() : RouteHelper.getLoginRoute()),
      MenuModel(
        icon: Images.editProfile,
        title: getTranslated('member_network', context),
        route: () => isLoggedIn ? RouteHelper.getMemberRoute() : RouteHelper.getLoginRoute(),
        iconWidget: Icon(Icons.groups_outlined, size: 50, color: Theme.of(context).textTheme.bodyLarge?.color),
      ),
      ...
      MenuModel(
        ...
        route: () => RouteHelper.getChatRoute(orderId: "", profileImage: "", userName: "", senderType: "admin"),
      ),
      ...
      if (splashProvider.configModel!.loyaltyPointStatus!) MenuModel(icon: Images.loyaltyPoint, title: getTranslated('loyalty_point', context), route: () => RouteHelper.getLoyaltyScreen()),
      ...
      if (splashProvider.configModel!.returnPolicyStatus!) MenuModel(icon: Images.returnPolicy, title: getTranslated('return_policy', context), route: () => RouteHelper.getReturnPolicyRoute()),

      if (splashProvider.configModel!.refundPolicyStatus!) MenuModel(icon: Images.refundPolicy, title: getTranslated('refund_policy', context), route: () => RouteHelper.getRefundPolicyRoute()),

      if (splashProvider.configModel!.cancellationPolicyStatus!) MenuModel(icon: Images.cancellationPolicy, title: getTranslated('cancellation_policy', context), route: () => RouteHelper.getCancellationPolicyRoute()),
      ...
      MenuModel(
        ...
        route: () => isLoggedIn ? showDialog(context: context, barrierDismissible: false, builder: (context) => const SignOutDialogWidget()) : RouteHelper.getLoginRoute(action: RouteAction.push),
      ),
    ];
                                ...
                                if (isLoggedIn)
                                  Column(
                                    children: [
                                      ...
                                      Text(profileProvider.userInfoModel?.email ?? '', style: poppinsRegular.copyWith(color: Theme.of(context).textTheme.bodyLarge?.color?.withValues(alpha: 0.6))),
                                    ],
                                  ),
                              ],
                            ),
                          ...
                          Builder(
                            builder: (context) {
                              return GridView.builder(
                                ...
                                shrinkWrap: true,
                                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 6, crossAxisSpacing: Dimensions.paddingSizeExtraLarge, mainAxisSpacing: Dimensions.paddingSizeExtraLarge),
